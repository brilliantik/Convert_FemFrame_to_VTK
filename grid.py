...
from FemFrameTool import read_net
...
class Grid:
	def __init__(self, vert: np.ndarray, elem_vert: list):
		...
		print(f"Считана сетка: {self.Nelem} elements, {self.Nvert} vertices")


def gu_build_from_net(filename: str) -> Grid:
	vert, elem_vert = read_net(filename)
...
def gu_volume_center(grid: Grid) -> (np.ndarray, np.ndarray):
	...
	for i in range(grid.Nelem):
		...
		# if area == 0:
		#     print('Error')
		#     print(grid.vert[vertexes[0]], grid.vert[vertexes[j]], grid.vert[vertexes[j + 1]])
		#     print(i)
		#     print('-----')
		volumes[i] = area
